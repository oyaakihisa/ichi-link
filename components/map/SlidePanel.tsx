...
function getPOITypeBadge(type: string): {
  label: string;
  color: string;
  bgColor: string;
} {
...
export function SlidePanel({
  ...
  const mode: PanelMode = selectedPoi
    ? "poi"
    : conversionResult
      ? "conversion"
      : "pin";
  ...
  const tokyoCoord = conversionResult
    ...
    : pin?.tokyoCoordinate;
  const address =
    selectedPoi?.address || conversionResult?.address || pin?.address;
  ...
  const handleCopyAll = useCallback(async () => {
    if (!wgs84Coord || !tokyoCoord) return;
    const text = generateFullCopyText(
      wgs84Coord,
      tokyoCoord,
      googleMapsUrl,
      address,
          ...
          {mode === "poi" && selectedPoi && (
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-2">
                <span
                  className={`px-2 py-0.5 text-xs font-medium rounded ${getPOITypeBadge(selectedPoi.type).bgColor} ${getPOITypeBadge(selectedPoi.type).color}`}
                >
